 ...
 // Hitung nomor antrian ramah (N-00N) dari no_resep yang dipanggil:
 // urutan resep sejalur (non-racikan) hari ini berdasarkan jam_peresepan, no_resep.
                    ...
                    <tr border='0'>
                        ...
                        <?php 
                            $_sql="select a.* from antriapotek2 a
                                    ...
                                    where a.no_resep not in(select distinct resep_dokter_racikan.no_resep from resep_dokter_racikan)
                                    and rp.kd_poli<>'IGDK' and ro.tgl_peresepan=CURDATE()" ;
                            $hasil=bukaquery($_sql);
                            while ($data = mysqli_fetch_array ($hasil)){
                                $rank=rankFarmasi($data['no_resep'],false);
                                $noantri=$rank>0?sprintf("N-%03d",$rank):$data['no_resep'];
                        ...
                        <?php 
                            ...
                            while ($data = mysqli_fetch_array ($hasil)){
                                ...
                                if($data['status']=="1"){
                                    ...
                                    $ucap="Nomor antrian non racikan nomor ".$rank.", ".$nmpsn.", silakan mengambil obat di loket penyerahan";
