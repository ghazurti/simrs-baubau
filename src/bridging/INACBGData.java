...
//                                        "        },\n" +
                ...
                // Fallback: kalau ditolak karena diagnosa/procedure (E2044),
                // kirim ulang tanpa 4 field tsb supaya tanggal, tarif, DPJP tetap masuk,
                // lalu kirim diagnosa & procedure satu per satu via add_diagnosa / add_procedure.
                if(response.has("metadata")
                        && response.getJSONObject("metadata").getInt("code")!=200
                        && response.getJSONObject("metadata").has("message")
                        && response.getJSONObject("metadata").getString("message").toLowerCase().contains("diagnosa")){
                    System.out.println(">> Retry set_claim_data tanpa diagnosa/procedure...");
                    String retryJson = request
                            .replaceAll("\\s*\"diagnosa\":\\s*\"[^\"]*\",\\n", "")
                            .replaceAll("\\s*\"procedure\":\\s*\"[^\"]*\",\\n", "")
                            .replaceAll("\\s*\"diagnosa_inagrouper\":\\s*\"[^\"]*\",\\n", "")
                            .replaceAll("\\s*\"procedure_inagrouper\":\\s*\"[^\"]*\",\\n", "");
                    System.out.println("retry JSON : "+retryJson);
                    response = inacbg.request(retryJson);
                    System.out.println("response retry :\n "+response);

                    // Pola PHP wsinacbg2.php: kirim ke IDRG dulu (idrg_diagnosa_set / idrg_procedure_set),
                    // grouping IDRG, lalu import ke INACBG (idrg_to_inacbg_import),
                    // baru kirim ke inacbg_diagnosa_set / inacbg_procedure_set.
                    String sep = Tsep.getText();
                    String diagStr = icd10.getText().trim();
                    String procStr = icd9.getText().trim();

                    // Step 1: IDRG diagnosa set (reset + isi)
                    if(!diagStr.isEmpty()){
                        try {
                            inacbg.request("{\"metadata\":{\"method\":\"idrg_diagnosa_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"diagnosa\":\"#\"}}");
                            System.out.println("idrg_diagnosa_set : "+diagStr);
                            JSONObject r = inacbg.request("{\"metadata\":{\"method\":\"idrg_diagnosa_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"diagnosa\":\""+diagStr+"\"}}");
                            System.out.println("resp idrg_diagnosa_set : "+r);
                        } catch(Exception ex){ System.out.println("Gagal idrg_diagnosa_set : "+ex.getMessage()); }
                    }

                    // Step 2: IDRG procedure set (reset + isi)
                    if(!procStr.isEmpty()){
                        try {
                            inacbg.request("{\"metadata\":{\"method\":\"idrg_procedure_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"procedure\":\"#\"}}");
                            System.out.println("idrg_procedure_set : "+procStr);
                            JSONObject r = inacbg.request("{\"metadata\":{\"method\":\"idrg_procedure_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"procedure\":\""+procStr+"\"}}");
                            System.out.println("resp idrg_procedure_set : "+r);
                        } catch(Exception ex){ System.out.println("Gagal idrg_procedure_set : "+ex.getMessage()); }
                    }

                    // Step 3: IDRG grouper
                    try {
                        System.out.println("idrg grouper stage 1");
                        JSONObject rIdrg = inacbg.request("{\"metadata\":{\"method\":\"grouper\",\"stage\":\"1\",\"grouper\":\"idrg\"},\"data\":{\"nomor_sep\":\""+sep+"\"}}");
                        System.out.println("resp idrg grouper : "+rIdrg);
                    } catch(Exception ex){ System.out.println("Gagal idrg grouper : "+ex.getMessage()); }

                    // Step 4: Import IDRG → INACBG
                    try {
                        System.out.println("idrg_to_inacbg_import");
                        JSONObject rImp = inacbg.request("{\"metadata\":{\"method\":\"idrg_to_inacbg_import\"},\"data\":{\"nomor_sep\":\""+sep+"\"}}");
                        System.out.println("resp idrg_to_inacbg_import : "+rImp);
                    } catch(Exception ex){ System.out.println("Gagal idrg_to_inacbg_import : "+ex.getMessage()); }

                    // Step 5: INACBG diagnosa set (reset + isi)
                    if(!diagStr.isEmpty()){
                        try {
                            inacbg.request("{\"metadata\":{\"method\":\"inacbg_diagnosa_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"diagnosa\":\"#\"}}");
                            System.out.println("inacbg_diagnosa_set : "+diagStr);
                            JSONObject r = inacbg.request("{\"metadata\":{\"method\":\"inacbg_diagnosa_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"diagnosa\":\""+diagStr+"\"}}");
                            System.out.println("resp inacbg_diagnosa_set : "+r);
                        } catch(Exception ex){ System.out.println("Gagal inacbg_diagnosa_set : "+ex.getMessage()); }
                    }

                    // Step 6: INACBG procedure set (reset + isi)
                    if(!procStr.isEmpty()){
                        try {
                            inacbg.request("{\"metadata\":{\"method\":\"inacbg_procedure_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"procedure\":\"#\"}}");
                            System.out.println("inacbg_procedure_set : "+procStr);
                            JSONObject r = inacbg.request("{\"metadata\":{\"method\":\"inacbg_procedure_set\",\"nomor_sep\":\""+sep+"\"},\"data\":{\"procedure\":\""+procStr+"\"}}");
                            System.out.println("resp inacbg_procedure_set : "+r);
                        } catch(Exception ex){ System.out.println("Gagal inacbg_procedure_set : "+ex.getMessage()); }
                    }
                }
...
//        CodeInaCBG.setText("");
    ...
    public void setDiagnosa(String norw, String norm, String pasien) {
        // Warning khusus RANAP: cek apakah pasien sudah dipulangkan dari kamar inap
        try {
            String sttsLanjut = Sequel.cariIsi("select status_lanjut from reg_periksa where no_rawat='"+norw+"'");
            if ("Ranap".equalsIgnoreCase(sttsLanjut)) {
                String tglKeluarRanap = Sequel.cariIsi("select tgl_keluar from kamar_inap where no_rawat='"+norw+"' order by tgl_masuk desc limit 1");
                boolean sudahPulang = tglKeluarRanap != null && !tglKeluarRanap.isEmpty() && !tglKeluarRanap.equals("0000-00-00");

                if (!sudahPulang) {
                    int pilih = JOptionPane.showConfirmDialog(this,
                        "PERHATIAN!\n\nPasien RANAP "+pasien+" ("+norm+")\nbelum dipulangkan dari kamar inap.\n\n"
                        + "Klaim INA-CBG sebaiknya diproses setelah pasien pulang\n"
                        + "agar tarif & komponen biaya sudah final.\n\n"
                        + "Lanjut buka form klaim?",
                        "Pasien Ranap Belum Dipulangkan",
            ...
            System.out.println("Gagal cek status pulang ranap: "+ex);
        }
