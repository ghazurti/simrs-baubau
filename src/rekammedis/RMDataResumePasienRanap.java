...
public final class RMDataResumePasienRanap extends javax.swing.JDialog {
    ...
    */
    ...
    public void setNoRm(String norwt, Date tgl2) {
        ...
        // Auto-isi keluhan, pemeriksaan, penilaian, alergi dari pemeriksaan_ranap
        try {
            ps=koneksi.prepareStatement(
        ...
        if(ObatSelamaDiRS.getText().isEmpty()){
            try {
                ps=koneksi.prepareStatement(
                    "select group_concat(concat(databarang.nama_brng,' ',detail_pemberian_obat.jml,' ',databarang.kode_sat) separator ', ') as obat "+
                ...
                try {
                    ps.setString(1,norwt);
                    rs=ps.executeQuery();
                    if(rs.next() && rs.getString("obat")!=null)
                        ObatSelamaDiRS.setText(rs.getString("obat"));
                } catch (Exception e) {
                    System.out.println("Notif : "+e);
                } finally {
                    if(rs!=null) rs.close();
        ...
        if(ObatPulang.getText().isEmpty()){
            try {
                ...
                try {
                    ...
                    rs=ps.executeQuery();
                    if(rs.next() && rs.getString("obat")!=null)
                        ObatPulang.setText(rs.getString("obat"));
                } catch (Exception e) {
                    System.out.println("Notif : "+e);
                } finally {
                    if(rs!=null) rs.close();
